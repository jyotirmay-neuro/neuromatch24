...
class Swim(swimmer.Swimmer):
    ...
    def before_step(self, action, physics):
        ...
        # hack
        self._visc_forces = self.viscosity_spheres.apply_forces(physics)
    ...
    def get_observation(self, physics):
        ...
        obs["visc_forces"] = self._visc_forces if self._visc_forces is not None else self.viscosity_spheres.apply_forces(physics)
        return obs
...
def load_env(**kwargs):
    return suite.load("swimmer", "swim_in_viscous_spheres2", task_kwargs={"random": 1, **kwargs})